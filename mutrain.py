...
def main(_):
	...
	for epoch in range(300000):
	    ...
	    #print batch_xs.shape
            ...
            summary,_,closs,gloss,dloss,smloss,drloss,sdecay,tdecay=sess.run([merged,optimizer,model.loss,model.G_loss,model.D_loss,model.Semanticloss,model.Drloss,model.source_decay,model.target_decay], feed_dict={x_s: batch_xs,x_t: Tbatch_xs,time:[1.0*gd],decay_learning_rate:rate,adlamb:lamb,is_training:True,y: batch_ys,dropout_keep_prob:0.5,yt:Tbatch_ys})
		...
		epoch=gd/(2000/128)
	        print 'lambda: ',lamb
	        print 'rate: ',rate
		print 'Epoch {6:<10} Step {3:<10} C_loss {0:<10} G_loss {1:<10} D_loss {2:<10} Sem_loss {4:<10} Dr_loss {5:<10}'.format(closs,gloss,dloss,gd,smloss,drloss,epoch)
		...
		print 'test_iter ',len(TEST.labels)
		    ...
		    #print TEST.pointer,'   ',TEST.shuffle
                    acc = sess.run(correct, feed_dict={x_t: batch_tx, yt: batch_ty, is_training:False,dropout_keep_prob: 1.})
                    test_acc += acc
                    test_count += 100
		...
		if gd%1000000==0:
		    nn=2000
		    batch_xs = TRAIN.images
                    Tbatch_xs = VALID.images
                    features=sess.run(model.concat_feature,feed_dict={x_s:batch_xs,x_t:Tbatch_xs,is_training:True,dropout_keep_prob:1.0})
                    labels=np.argmax(batch_ys,1).tolist()+np.argmax(Tbatch_ys,1).tolist()

                    domain_labels=np.array((2000)*[1]+1800*[0])
                    #print domain_labels
                    red= domain_labels==0
                    green= domain_labels==1
                    #print red
                    #print green

                    tsne=manifold.TSNE(n_components=2,init='random',random_state=0,perplexity=35)
                    #(fig,subplots)=plt.subplots(3,5,figsize=(15,8))
                    fig=plt.figure()
                    ax=fig.add_subplot(111)
                    Y=tsne.fit_transform(features)
                    ax.scatter(Y[red,0],Y[red,1],c='r',s=5)
                    ax.scatter(Y[green,0],Y[green,1],c='b',s=5)
                    ax.axis('off')
                    plt.show()
